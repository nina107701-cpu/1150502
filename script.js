...
const DATA = {
  ...
  work:[{ title:'效率開光', badgeLabel:'工作運', energy:80, message:'工作節奏順，靈感與執行力都在線。', action:'把最難的一件事先完成。', avoid:'別同時開太多戰線。', blessing:'你專注的樣子，正在發光。'},{ title:'協作有利', badgeLabel:'工作運', energy:67, message:'今天適合溝通與協作，單打獨鬥反而慢。', action:'主動確認彼此期待，會少很多誤會。', avoid:'不要悶著不問。', blessing:'善意交流會替你開路。'},{ title:'低調守成', badgeLabel:'工作運', energy:55, message:'適合穩穩做，不宜冒進。', action:'先把既有流程補強。', avoid:'不要衝動答應超出負荷的要求。', blessing:'保留餘裕，就是成熟。'}],
  soul:[{ title:'把心放回來', badgeLabel:'心靈指引', energy:74, message:'你值得先照顧自己，再照顧世界。', action:'給自己 20 分鐘安靜時間。', avoid:'別把所有責任都攬在身上。', blessing:'你被溫柔對待，也很重要。'},{ title:'允許慢下來', badgeLabel:'心靈指引', energy:66, message:'慢不是停滯，而是重新對焦。', action:'今天少做一件不必要的事。', avoid:'不要拿效率否定價值。', blessing:'你的存在，本來就有重量。'},{ title:'鬆開執念', badgeLabel:'心靈指引', energy:52, message:'有些答案需要空間才會出現。', action:'把放不下的事寫下來，暫時交給明天。', avoid:'別逼自己立刻想通。', blessing:'放鬆後，路會自己顯現。'}],
  cosmos:[{ title:'星軌對齊中', badgeLabel:'宇宙訊息', energy:77, message:'你正在接近一個對的轉彎。', action:'留意今天出現的巧合。', avoid:'不要忽略第一直覺。', blessing:'宇宙正在回應你。'},{ title:'月光蓄能夜', badgeLabel:'宇宙訊息', energy:64, message:'今晚適合整理心願，讓能量沉澱。', action:'睡前寫下一句你真正想成為的樣子。', avoid:'別把外界期待當成你的答案。', blessing:'你正在成為你想成為的人。'},{ title:'流星訊號弱', badgeLabel:'宇宙訊息', energy:50, message:'先穩定日常，宇宙訊號會更清楚。', action:'回到生活裡最基本的節奏。', avoid:'不要急著求快。', blessing:'慢慢來，也是一種被守護。'}]
};
DATA.love_brain = DATA.love_today.map((_,i)=>[{title:'高敏心動型',badgeLabel:'戀愛腦',energy:87,message:'你今天的戀愛腦指數偏高，特別容易因為一句話、一個眼神，甚至一個已讀時間就開始腦補後續劇情。',action:'把心動當成可愛的小訊號就好，不用急著立刻下結論。',avoid:'避免在情緒高點時連發訊息，或自己腦內加戲到睡不著。',blessing:'你會心動，代表你心裡還有光。'},{title:'理智微失守',badgeLabel:'戀愛腦',energy:72,message:'你表面還算冷靜，但只要碰到在意的人，理智其實很容易鬆開一點點。今天的你會比平常更在意對方的反應。',action:'承認自己在意，反而會讓你更穩。',avoid:'不要假裝完全不在乎，最後卻偷偷反覆打開聊天框。',blessing:'有時候承認喜歡，不是輸，是誠實。'},{title:'半糖清醒型',badgeLabel:'戀愛腦',energy:58,message:'你今天的戀愛腦不算重，會心動，但還留得住自己的步調。這是很剛好的狀態，既能感受，也不容易失控。',action:'用輕鬆的方式相處，讓感情自然流動。',avoid:'別突然切太冷，讓對方看不懂你的溫度。',blessing:'會愛人，也懂得愛自己，是很難得的平衡。'},{title:'已讀放大型',badgeLabel:'戀愛腦',energy:81,message:'你今天很容易把一個小小的回覆節奏無限放大。對方晚回一點，你心裡就已經演到第八集。',action:'當你想重看聊天紀錄第三次時，先去做別的事 10 分鐘。',avoid:'不要拿對方的回覆速度，直接判定自己的重要程度。',blessing:'你的在意是真的，但不需要用焦慮證明。'},{title:'表面冷靜派',badgeLabel:'戀愛腦',energy:65,message:'你今天看起來很淡定，實際上只是把情緒藏得很好。內心不是沒波動，而是偷偷翻湧。',action:'允許自己承認：對，你就是有點在意。',avoid:'不要用過度理性壓住所有真實感受。',blessing:'能感受到喜歡，本來就是一種很柔軟的能力。'},{title:'深夜加戲型',badgeLabel:'戀愛腦',energy:84,message:'白天還好，一到夜裡你的戀愛腦就會特別活躍，開始反覆回想對話、表情和語氣。',action:'睡前把你真正擔心的事寫下來，會比一直想更有用。',avoid:'不要在半夜情緒最滿的時候做重大感情決定。',blessing:'夜晚會放大情緒，但天亮後你還是會找到平衡。'},{title:'嘴硬心軟型',badgeLabel:'戀愛腦',energy:69,message:'你今天的戀愛腦藏在嘴硬裡，明明很在意，卻總想裝得一副還好。這讓你的可愛有點被誤會。',action:'選一個安全的小方式示弱，反而能讓互動更自然。',avoid:'不要老用反話測試對方，最後連自己都委屈。',blessing:'被看懂的你，會比你想像中更值得被疼。'},{title:'想太多預備軍',badgeLabel:'戀愛腦',energy:77,message:'你今天的腦袋很容易提前預演失敗結局，還沒發生的事已經先替自己受傷一遍。',action:'提醒自己：感覺是真的，但猜測不一定是真的。',avoid:'別用最壞劇本嚇退自己原本想要的靠近。',blessing:'你值得一段不用一直靠猜的關係。'},{title:'穩穩喜歡型',badgeLabel:'戀愛腦',energy:54,message:'今天的你比較像穩穩喜歡型，心裡有暖度，但不會失控。這讓你在感情裡特別有魅力。',action:'繼續保持自己的步調，不用刻意表演熱情。',avoid:'不要因為別人很熱烈，就懷疑自己的喜歡不夠真。',blessing:'平靜的喜歡，也可以很深。'},{title:'一秒心軟型',badgeLabel:'戀愛腦',energy:89,message:'只要對方稍微釋出一點好意，你今天就很容易整顆心軟下來。這份柔軟很可愛，但也很需要保護。',action:'先享受被在意的感覺，但記得觀察對方是否真的穩定。',avoid:'不要因為一時甜，就忽略本來存在的問題。',blessing:'願你的心軟，總是被值得的人接住。'}][i]);
DATA.love_think = [
...
const state={mode:'home', category:null, topic:null};
...
function setNav(active){ document.querySelectorAll('.nav-btn').forEach(b=>b.classList.toggle('active',b.dataset.nav===active)); }
function renderHome(){ setNav('home'); app.innerHTML=`<section class="card"><h1 class="title">小喵子月蝕占卜室</h1><p class="subtitle">聽見命運給你的回音</p><img class="emblem" src="assets/xiaomiaozi-emblem.svg" alt="小喵子吉祥物"><div class="grid"><button class="option-btn" data-cat="love">戀愛運勢</button><button class="option-btn" data-cat="daily">每日凶吉</button><button class="option-btn" data-cat="work">工作運勢</button><button class="option-btn" data-cat="soul">心靈指引</button></div></section>`; }
function renderLoveTopics(){ app.innerHTML=`<section class="card"><h2>戀愛運勢 · 選題</h2><div class="grid"><button class="option-btn" data-topic="love_today">今日戀愛占卜</button><button class="option-btn" data-topic="love_brain">你的戀愛腦指數</button><button class="option-btn" data-topic="love_think">他有偷偷想我嗎?</button></div><button class="primary-btn" id="go-home">返回首頁</button></section>`; }
function renderRitual(topic){ const name=TOPICS[topic]||'儀式'; app.innerHTML=`<section class="card ritual-wrap"><h2>${name}</h2><p class="small">把指尖放在水晶上，長按 2 秒，讓月光回應你。</p><div class="hold-area"><div class="hold-ring" id="hold-ring"><button class="hold-btn" id="hold-btn"><img src="assets/crystal.svg" alt="水晶"></button></div></div><div class="progress-text" id="progress-text">按住啟動儀式</div><button class="primary-btn" id="back-topic">重選題目</button></section>`; attachHold(topic); }
function renderResult(topic,r){ app.innerHTML=`<section class="card result-section"><p class="small">題目：${TOPICS[topic]}</p><span class="result-badge">${r.badgeLabel}</span><h2 class="result-title">${r.title}</h2><div class="result-energy">能量指數：${r.energy}%</div><p><strong>核心訊息：</strong>${r.message}</p><p><strong>適合行動：</strong>${r.action}</p><p><strong>避開事項：</strong>${r.avoid}</p><p><strong>祝福句：</strong>${r.blessing}</p><div class="grid"><button class="primary-btn" id="retry">再抽一次</button><button class="option-btn" id="retopic">重選題目</button></div></section>`;
...
function attachHold(topic){ const btn=document.getElementById('hold-btn'); const ring=document.getElementById('hold-ring'); const text=document.getElementById('progress-text'); let t0=0,raf=0,holding=false;
 const start=(e)=>{e.preventDefault();holding=true;t0=performance.now();tick();};
 const stop=()=>{holding=false;cancelAnimationFrame(raf);ring.style.setProperty('--progress',0);text.textContent='按住啟動儀式';};
 const tick=(now=performance.now())=>{ if(!holding) return; const p=Math.min(100,Math.floor((now-t0)/20)); ring.style.setProperty('--progress',p); text.textContent=`儀式連結中 ${p}%`; if(p>=100){holding=false;renderResult(topic,pick(DATA[topic]));return;} raf=requestAnimationFrame(tick); };
 ['mousedown','touchstart','pointerdown'].forEach(evt=>btn.addEventListener(evt,start,{passive:false})); ['mouseup','mouseleave','touchend','touchcancel','pointerup','pointercancel'].forEach(evt=>btn.addEventListener(evt,stop));
...
document.addEventListener('click',(e)=>{
  ...
  if(t.matches('[data-nav="ritual"]')) return renderLoveTopics(),setNav('ritual');
  if(t.matches('[data-nav="cosmos"]')) return setNav('cosmos'),renderRitual('cosmos');
  if(t.matches('[data-nav="wish"]')) return setNav('wish'),modal.classList.remove('hidden');
});
